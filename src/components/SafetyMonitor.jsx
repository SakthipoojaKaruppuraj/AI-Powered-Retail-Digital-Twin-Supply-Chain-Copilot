...
import { ShieldCheck, ShieldAlert, UserCheck, XCircle } from 'lucide-react';
import { api } from '../services/api';

export default function SafetyMonitor({ alerts: rawAlerts = [], onResolveAlert }) {
  const [safetyIntel, setSafetyIntel] = useState(null);

  // Fetch backend-calculated safety intelligence from REST API
  useEffect(() => {
    const fetchSafetyData = async () => {
      try {
        const data = await api.getSafetyIntelligence();
        if (data) {
          setSafetyIntel(data);
        }
      } catch (err) {
        console.error('Failed to fetch safety intelligence from REST API:', err);
      }
    };
    fetchSafetyData();
  }, [rawAlerts]);

  const activeAlerts = safetyIntel?.alerts || rawAlerts.map(a => ({
    id: a.id,
    text: a.text,
    severity: (a.severity || 'medium').toLowerCase(),
    time: a.time || 'Recent',
    zone: a.zone || 'Warehouse Floor'
  }));
    ...
    <div className="glass-panel p-6 rounded-2xl flex flex-col h-full">
      ...
      <div className="flex justify-between items-start mb-4">
        <div>
          ...
          <p className="text-xs text-[#2a3723]/70 mt-0.5 font-medium font-sans">Automated computer vision alerts for safety compliance issues</p>
        </div>
                ...
                <div className="flex items-start gap-2.5">
                  <span className="mt-0.5">
                    {sev === 'critical' ? (
                      ...
                      <ShieldAlert className={`w-4 h-4 shrink-0 ${sev === 'high' ? 'text-rose-650' : 'text-amber-600'}`} />
                    )}
                  </span>
                  <div>
